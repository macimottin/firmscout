...
| `govulncheck` | **Implemented** | Runs on every push and pull request, pinned to `v1.1.4`. The earlier `go.mod` existence guard was removed: it allowed the job to pass by doing nothing, so a missing module is now a hard failure with an explicit annotation |
