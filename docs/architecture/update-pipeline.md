...
| 3 | Source compliance | The source's current health and compliance status (`robots_policy_status` in `allowed`/`not_applicable`, `terms_review_status` in `approved`/`restricted`) both hold at validation time, not merely at collection time — and, since the fix pass, at publication time too, so a takedown recorded while a candidate sat in the review queue still stops it | Source is not `active`, or compliance status has since changed to disqualifying | — |
