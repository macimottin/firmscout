...
- **`robots_policy_status`** — a mechanical fact obtained by fetching and parsing the host's `robots.txt`: `allowed`, `disallowed`, or `not_checked`. This is not a judgment call; it is the literal, verifiable result of the check, and it cannot be argued with the way a terms-of-service reading can.
- **`terms_review_status`** — a human judgment, often needing legal input: `pending`, `reviewed_ok`, or `reviewed_blocked`.

**A source is enabled for automated collection only when `robots_policy_status = allowed` AND `terms_review_status = reviewed_ok`.** Either field short of that, and the source stays registered — visible, documented, in the dataset — but `enabled = false`. This is why Dell's `downloads.dell.com/catalog/Catalog.xml.gz`, a technically ideal, ETag-bearing, well-formed catalogue, sits in the registry uncollected: its `robots.txt` returns `Disallow: /` for all user agents. FirmScout's own stated policy is not suspended for its most convenient source (blueprint §3.6) — if it were, the policy would mean nothing for the next thousand sources that are less convenient and less tempting to bend the rule for.
