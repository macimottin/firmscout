...
sequenceDiagram
    ...
    alt Not modified
        ...
        Worker->>Validator: Validate candidates
        alt Deterministic validation passes
            ...
            Worker->>AI: Request repair or classification proposal
...
The overwhelming majority of scheduled checks take the cheap left branch: a conditional GET returns `304 Not Modified`, the check is recorded, and the source is rescheduled — no artifact storage, no extraction, no AI. Only a `200 OK` with an actually-changed body triggers the expensive branch: artifact storage, extraction into candidates, and deterministic validation. Validation itself branches again — most candidates pass deterministic rules and publish directly; only ambiguous or low-confidence candidates escalate to AI, and even then AI never publishes — it produces a proposal that lands in the review queue alongside deterministic escalations.
...
- Extraction can throw on a genuinely malformed artifact (a config selector matching zero elements after a layout change) — this must produce a `SourceBroken`-style outcome and a repair-triggering event, not a silent empty candidate list.
...
- `internal/application/checksource.go` and `ingest.go` — the sequence itself
- `internal/adapters/collectors` — the extraction step
- `internal/integration/slice_test.go` — the sequence run end to end
