...
**Dell stays registered and disabled.** The vendor and the catalogue source are recorded in the registry with `robots_policy_status = 'disallowed'`, `terms_review_status = 'pending'`, and `enabled = false`. *(This sentence read "the vendor, its products, and the catalogue source" until the follow-through note below. No Dell product is registered, and none honestly can be: establishing which products that catalogue covers means reading the catalogue, which is the fetch `robots.txt` forbids. The source is registered catalogue-scoped, with `Source.ProductID` empty — the shape the domain already models for "covers a family or a whole catalogue".)* This is deliberately more useful than deleting it: the record documents that the source exists, that it was evaluated, and why it is not collected, so the question is not silently re-opened every few months by someone who rediscovers the catalogue. The path forward is Dell's official APIs or written permission, not a quiet policy exception.
...
Phase 2's closing pass registered the three pilot vendors this ADR describes and, in doing so, turned two of its sentences from claims into checkable facts and found a third that was wrong.

**The registry rows now exist.** `dataset/vendors/dell.yaml` + `dataset/sources/dell/catalog.yaml`, `dataset/vendors/poly.yaml` + `dataset/sources/poly/documentation-portal.yaml`, and `dataset/vendors/ubiquiti.yaml` + `dataset/sources/ubiquiti/firmware-latest.yaml`. Until this pass, `DATA_SOURCES.md` and this ADR both asserted Dell was "registered and disabled" while no such row existed anywhere in `dataset/`. The flagship demonstration of the policy was described but not performed. `go run ./apps/cli registry validate` now reports *Registry is valid: 5 vendor(s), 7 category(ies), 0 family(ies), 2 product(s), 6 source(s). 0 of 6 source(s) are currently collectable*, and `registry sync` names robots.txt as one of Dell's reasons and only Dell's.

**"its products" was inaccurate and has been corrected in place, with the reason.** See the parenthetical above.

**The provenance of this ADR's own Dell measurements is a tension worth naming.** The paragraph above beginning "This stopped being hypothetical" states measured facts about `downloads.dell.com/catalog/Catalog.xml.gz` — 1.4 MB, well-formed, serving both `ETag` and `Last-Modified`, covering BIOS, drivers and firmware. Those facts can only have been obtained by fetching the path this ADR then declares off-limits. They are left in place with their original 2026-09-03 attribution rather than quietly refreshed or quietly deleted: the closing pass deliberately did **not** re-request that path in any form — not a `GET`, not a `HEAD`, not a ranged request — so anything here that could only come from the body is unverified as of today and is marked as such in `DATA_SOURCES.md`. The honest reading is that the evaluation which produced this policy predates the policy. That is normal for a first compliance decision and it is not a licence to repeat it; what a future evaluation of a disallowed path may record is the robots verdict and nothing that requires the body.

**Dell's `authentication_type: none` is a schema default, not a measurement.** Establishing whether the catalogue demands credentials means requesting the disallowed path. The vocabulary (`none|api_key|account_required|entitlement_required`) has no member meaning "not established", so the row carries the default and says so in a comment. It changes nothing operationally — the row is undispatchable on robots alone — but a vocabulary that cannot express "unknown" is a gap in `internal/domain/source.go` and the migration CHECK, recorded here rather than papered over.
